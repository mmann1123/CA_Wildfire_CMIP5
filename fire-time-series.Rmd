...
```{r}
  ...
  pop_scen = 'bau'
  clim_scen = '45'  # '85|45' to get both

  search_range = paste(seq(decade_group_names[[1]][1],decade_group_names[[1]][2]),collapse='|')
  mean_base_year = fire_tif %>% filter(grepl(search_range,fire_tif))%>% filter(grepl(pop_scen,fire_tif)) %>% .$fire_tif %>% stack() %>% mean(.,na.rm=T)
  
  # create stacks by decade group 
  search_range = paste(seq(decade_group_names[[2]][1],decade_group_names[[2]][2]),collapse='|')
  stack_2001_2025 = fire_tif %>% filter(grepl(search_range,fire_tif))%>% filter(grepl(pop_scen,fire_tif))%>% filter(grepl(clim_scen,fire_tif)) %>% .$fire_tif %>% stack()  
  ...
  threshold = 1  # percentage that should agree eg 0.75
  threshold_n = round(dim(get(ls(envir=.GlobalEnv, pattern="dif_")[1]))[3]*threshold)
  for(dif_name in ls(envir=.GlobalEnv, pattern="dif_")){
    print(dif_name)
  ...
  plot(agree_dif_2001_2025,main='agree_100perct_2001_2025')
  plot(agree_dif_2026_2050,main='agree_100perct_2026_2050')
  plot(agree_dif_2051_2075,main='agree_100perct_2051_2075')
  plot(agree_dif_2076_2100,main='agree_100perct_2076_2100')

  # convert to data frame and set MFRI labels 
  for(dif_name in ls(envir=.GlobalEnv, pattern="agree_dif_")){
      df = data.frame(rasterToPoints(get(dif_name)))
      df$MFRI =   revalue(as.character(df$layer), c('-1'='+ MFRI', '0'='None','1'='- MFRI'))  # 1 is more fire
  ...
  # plot agreement  
  ggplot() + geom_raster(data=na.omit(df_agree_dif_2001_2025), aes(x=x, y=y, fill=factor(MFRI)))  + geom_sf(data=CC4a_reg,colour = "grey30", fill = NA,size=.75) +   coord_sf( xlim=c(aoi[1],aoi[3]),ylim=c(aoi[2],aoi[4]) )+ scale_fill_manual(values = c( "#31B404","grey","#DF013A" ),name="Agreement")  + labs(fill='Human Effect\non Fire Count') +ggtitle('2001_2025')
  ggsave(paste(pop_scen,clim_scen,'2001_2025_Westerling_agreement.png',sep='_'))
  
  ggplot() + geom_raster(data=na.omit(df_agree_dif_2026_2050), aes(x=x, y=y, fill=factor(MFRI)))  + geom_sf(data=CC4a_reg,colour = "grey30", fill = NA,size=.75) +   coord_sf( xlim=c(aoi[1],aoi[3]),ylim=c(aoi[2],aoi[4]) )+ scale_fill_manual(values = c("#31B404","grey","#DF013A"),name="Agreement")  + labs(fill='Human Effect\non Fire Count') +ggtitle('2026_2050')
  ggsave(paste(pop_scen,clim_scen,'2026_2050_Westerling_agreement.png',sep='_'))

  ggplot() + geom_raster(data=na.omit(df_agree_dif_2051_2075), aes(x=x, y=y, fill=factor(MFRI)))  + geom_sf(data=CC4a_reg,colour = "grey30", fill = NA,size=.75) +   coord_sf( xlim=c(aoi[1],aoi[3]),ylim=c(aoi[2],aoi[4]) )+ scale_fill_manual(values = c("#31B404","grey","#DF013A"),name="Agreement")  + labs(fill='Human Effect\non Fire Count') +ggtitle('2051_2075')
  ggsave(paste(pop_scen,clim_scen,'2051_2075_Westerling_agreement.png',sep='_'))

    ggplot() + geom_raster(data=na.omit(df_agree_dif_2076_2100), aes(x=x, y=y, fill=factor(MFRI)))  + geom_sf(data=CC4a_reg,colour = "grey30", fill = NA,size=.75) +   coord_sf( xlim=c(aoi[1],aoi[3]),ylim=c(aoi[2],aoi[4]) )+ scale_fill_manual(values = c("#31B404","grey","#DF013A"),name="Agreement")  + labs(fill='Human Effect\non Fire Count') +ggtitle('2076_2100')
   ggsave(paste(pop_scen,clim_scen,'2076_2100_Westerling_agreement.png',sep='_'))
