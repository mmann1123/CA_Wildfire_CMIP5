...
```{r}
  ...
  pop_scen = 'bau'
  clim_scen = '_45_'  # '_85_|_45_' to get both
  ...
  for(dif_name in ls(envir=.GlobalEnv, pattern="dif_model")){
   ...
   # NEED TO STACK dif_model_mean by period
   # create stacks by decade group 
  search_range = paste(seq(decade_group_names[[2]][1],decade_group_names[[2]][2]),collapse='|')
  ...
  threshold_n = round( dim(get( "stack_2076_2100"))[3]*threshold) # num that must agree
  for(dif_name in ls(envir=.GlobalEnv, pattern="^stack_")){   #^ is "starts with"
    ...
    agg_s[sum_s>threshold_n]  = 1 
    agg_s[sum_s<(-1*threshold_n)]  = -1
  ...
  for(dif_name in ls(envir=.GlobalEnv, pattern="agree_stack_")){
      ...
      if(length(unique(df$layer))==3){
          df$MFRI =   revalue(as.character(df$layer), c('-1'='+ MFRI', '0'='None','1'='- MFRI'))  
          # +1 is more fire -MFRI is more fire
          df$MFRI = ordered(df$MFRI, levels =c('+ MFRI','None','- MFRI'))
      } 
      if(length(unique(df$layer))==2){
          df$MFRI =   revalue(as.character(df$layer), c('-1'='+ MFRI', '0'='None' ))  
          # 1 is more fire
  ...
  ggsave(paste(pop_scen,clim_scen,'agree',threshold,paste('2001_2025_Westerling_agreement.png'),sep='_'))
  ...
  ggsave(paste(pop_scen,clim_scen,'agree',threshold,'2026_2050_Westerling_agreement.png',sep='_'))
  ...
  ggsave(paste(pop_scen,clim_scen,'agree',threshold,'2051_2075_Westerling_agreement.png',sep='_'))
   ...
   ggsave(paste(pop_scen,clim_scen,'agree',threshold,'2076_2100_Westerling_agreement.png',sep='_'))
